  ...
  <script>
    ...
    const memberCardMarkup=(title,member={})=>`<div class="aux-titlebar"><span>${title}</span><div class="window-controls"><button type="button" data-act="min">_</button><button type="button" data-act="max">□</button><button type="button" class="window-close" data-act="close">×</button></div></div><div class="member-card-content"><form class="member-card-form"><nav class="member-card-tabs" aria-label="Üye kartı bölümleri"><button type="button" class="active" data-member-tab="personal"><span class="member-card-tab-icon">ⓘ</span><span>Bilgiler</span></button><button type="button" data-member-tab="membership"><span class="member-card-tab-icon">§</span><span>Üyelik</span></button><button type="button" data-member-tab="accounting"><span class="member-card-tab-icon">₺</span><span>Muhasebe</span></button></nav><div class="member-card-toolbar"><button type="submit">💾 Kaydet</button><span class="member-card-status" role="status"></span></div><div class="member-card-hint">Üye bilgilerini düzenleyip Kaydet düğmesine basın.</div>
      ...
      <fieldset class="member-card-section member-card-panel" data-member-panel="membership" hidden><legend>ÜYELİK BİLGİLERİ / MEMBERSHIP INFO</legend><div class="member-card-grid">
        <label>Üyelik Türü / Membership Type</label><select name="membershipType" data-membership-package required><option value="${attr(member.membershipType)}">Üyelik paketleri yükleniyor…</option></select>
        <label>Süresi / Duration (Ay)</label><input name="durationMonths" type="number" min="0" value="${attr(member.durationMonths)}" readonly>
        <label>Başlangıç / Valid From</label><input name="validFrom" type="date" value="${attr(member.validFrom)}">
        <label>Bitiş / Through</label><input name="validThrough" type="date" value="${attr(member.validThrough)}" readonly>
      </div></fieldset>
    ...
    const setupReservationExtraServicePicker=form=>{
      ...
      const templates=reservationPackageTemplates(),selected=new Map(),record=reservationData.reservations.find(item=>Number(item.id)===Number(form.dataset.reservationRecordId)),body=form.querySelector('.reservation-extra-table tbody'),extraTable=body?.closest('table'),tools=form.querySelector('.reservation-extra-tools'),addButton=form.querySelector('[title="Ek hizmet ekle"]'),removeButton=form.querySelector('[title="Seçileni kaldır"]'),extraTitle=tools?.previousElementSibling,header=form.querySelector('.reservation-template-table thead th:first-child');
    ...
    const packagesMarkup=title=>`<div class="aux-titlebar"><span>${title}</span><div class="window-controls"><button type="button" data-act="min">_</button><button type="button" data-act="max">□</button><button type="button" class="window-close" data-act="close">×</button></div></div><div class="packages-shell"><div class="packages-toolbar"><button type="button" data-package-action="new">⊕ Yeni Paket</button><span class="packages-message" role="status"></span></div><div class="packages-body"><section class="packages-editor" data-package-editor hidden></section><p class="packages-intro">Sofitel Spa paketleri ve öne çıkan içerikleri</p><div class="packages-filters"><label for="package-service-group-filter">Hizmet Grubu</label><select id="package-service-group-filter" data-package-service-group-filter><option value="">Tüm hizmet grupları</option></select></div><div class="packages-table-wrap"><table class="packages-table"><thead><tr><th>Hizmet Grubu</th><th>Paket Türü</th><th>Ortalama Süre</th><th>Fiyat (€)</th><th>Öne Çıkan İçerikler</th><th>Kimler İçin Uygun?</th><th>İşlemler</th></tr></thead><tbody><tr><td colspan="7" class="stock-empty">Paketler yükleniyor…</td></tr></tbody><tfoot data-package-totals>${packageTotals([])}</tfoot></table></div></div></div>`;
    const setupPackageWindow=win=>{
      const message=win.querySelector('.packages-message'),editor=win.querySelector('[data-package-editor]'),tbody=win.querySelector('.packages-table tbody'),totals=win.querySelector('[data-package-totals]'),filter=win.querySelector('[data-package-service-group-filter]');
      const render=()=>{const selected=filter.value,records=selected==='ungrouped'?packageRecords.filter(item=>!item.service_group_id):selected?packageRecords.filter(item=>Number(item.service_group_id)===Number(selected)):packageRecords;tbody.innerHTML=packageRows(records);totals.innerHTML=packageTotals(records);message.textContent=`${records.length} / ${packageRecords.length} paket`;};
    ...
    const openAuxWindow = (id,title) => {
      ...
      if(id.startsWith('member-')){setupMemberPhoto(win);setupMemberMembership(win);const form=win.querySelector('.member-card-form'),status=win.querySelector('.member-card-status');win.querySelectorAll('[data-member-tab]').forEach(tab=>tab.onclick=()=>{win.querySelectorAll('[data-member-tab]').forEach(item=>item.classList.toggle('active',item===tab));win.querySelectorAll('[data-member-panel]').forEach(panel=>panel.hidden=panel.dataset.memberPanel!==tab.dataset.memberTab);});form.addEventListener('submit',async event=>{event.preventDefault();status.textContent='Kaydediliyor…';try{await saveMemberCard(id,form);status.textContent='Üye bilgileri kaydedildi.';}catch(error){status.textContent=error.message;}});}
    };
    ...
    const setupMemberPhoto=win=>{
      ...
      input.addEventListener('change',()=>{
        ...
        const reader=new FileReader();reader.onload=()=>{if(version!==selection)return;const img=document.createElement('img');img.src=reader.result;img.alt='Seçilen üye fotoğrafı';preview.replaceChildren(img);status.textContent='Fotoğraf hazır. Kaydet düğmesine basın.';};reader.readAsDataURL(file);
      });
    };
