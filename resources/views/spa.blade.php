...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; background:var(--canvas); color:var(--ink); font:14px/1.4 "Segoe UI", system-ui, sans-serif; }
    ...
    .reservation-day-events { display:grid; gap:6px; padding:8px; }
    .reservation-day-event { display:grid; grid-template-columns:85px minmax(140px,1fr) minmax(150px,1.2fr) minmax(130px,1fr) 90px; gap:8px; align-items:center; padding:9px; color:#2b261c; border:1px solid #c9ad67; border-left:5px solid #2e7460; background:#fffaf0; text-align:left; font:10px Tahoma; cursor:pointer; }.reservation-day-event.confirmed{border-left-color:#176a8b}.reservation-day-event.completed{border-left-color:#6e7c49}.reservation-day-event.cancelled{opacity:.7;text-decoration:line-through}.reservation-day-event.no_show{border-left-color:#a5483f}.reservation-day-event strong{color:#245849}
    .reservation-empty { padding:28px; color:#76694e; text-align:center; font:11px Tahoma; }
    ...
    .aux-window[data-window-id="calendar"]{min-width:800px;min-height:500px}@media(max-width:800px){.reservation-calendar,.reservation-week-calendar{min-width:805px}.reservation-day-event{grid-template-columns:75px 1fr}.reservation-form{grid-template-columns:115px 1fr}.reservation-form textarea{grid-column:auto}.reservation-filters{flex-direction:column}.reservation-filters select{width:100%}}
...
<body>
  ...
  <script>
    ...
    const reservationWeekMarkup=()=>{const range=reservationRange(),start=reservationParseDate(range.start),end=reservationAddDays(reservationParseDate(range.end),-1),today=localIso(new Date()),columns=Array.from({length:7},(_,index)=>{const current=reservationAddDays(start,index),date=localIso(current);return`<div class="reservation-week-column ${date===today?'today':''}"><div class="reservation-weekday">${reservationDateLabel(current,{weekday:'long'})}</div><div class="reservation-day-head"><span>${reservationDateLabel(current,{day:'2-digit',month:'2-digit'})}</span><button type="button" data-reservation-add="${date}" title="Rezervasyon ekle">＋</button></div>${reservationEventsFor(date).map(reservationEventButton).join('')}</div>`;}).join('');return`${reservationPeriodHeader(`${reservationDateLabel(start,{day:'2-digit',month:'long'})} – ${reservationDateLabel(end,{day:'2-digit',month:'long',year:'numeric'})}`)}<div class="reservation-week-calendar">${columns}</div>`;};
    const reservationDayMarkup=()=>{const anchor=reservationParseDate(reservationData.anchorDate),date=localIso(anchor),items=reservationEventsFor(date);return`${reservationPeriodHeader(reservationDateLabel(anchor,{weekday:'long',day:'2-digit',month:'long',year:'numeric'}))}<div class="reservation-day-view"><div class="reservation-day-title"><span>${items.length} rezervasyon</span><button type="button" data-reservation-add="${date}" title="Rezervasyon ekle">＋</button></div><div class="reservation-day-events">${items.length?items.map(item=>`<button type="button" class="reservation-day-event ${item.status}" data-reservation-id="${item.id}"><strong>${attr(String(item.start_time).slice(0,5))}–${attr(String(item.end_time).slice(0,5))}</strong><span>${attr(reservationGuest(item))}</span><span>${attr(item.service_name)}</span><span>${attr(reservationEmployee(item))}</span><span>${attr(reservationStatusName(item.status))}</span></button>`).join(''):'<div class="reservation-empty">Bu gün için rezervasyon bulunmuyor.</div>'}</div></div>`;};
