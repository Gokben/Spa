...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; background:var(--canvas); color:var(--ink); font:14px/1.4 "Segoe UI", system-ui, sans-serif; }
    ...
    .current-service-options { display:flex; align-items:center; gap:20px; min-height:63px; padding:8px; border:1px solid #c9ad67; background:#fff; }
    .current-service-options label { display:flex; align-items:center; gap:6px; margin:0; font-weight:bold; }
    .current-service-options input { width:18px; min-height:18px; accent-color:#0a9848; }
  ...
  <script>
    ...
    const setupSmsWindow=win=>{
      ...
      const render=data=>{
        ...
        logBody.innerHTML=messages.length?messages.map(item=>`<tr><td>${attr(item.sent_at?new Date(item.sent_at).toLocaleString('tr-TR'):'—')}</td><td>${attr(item.recipient_name||'—')}</td><td>${attr(item.destination)}</td><td title="${attr(item.message)}">${attr(item.message.length>70?item.message.slice(0,70)+'…':item.message)}</td><td>${attr(item.campaign_id||'—')}</td><td>${attr(statusLabel(item.status))}</td></tr>`).join(''):'<tr><td colspan="6">Henüz SMS gönderim kaydı bulunmuyor.</td></tr>';
