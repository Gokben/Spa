...
<body>
  ...
  <script>
    ...
    loginForm.addEventListener('submit',async event=>{event.preventDefault();const email=loginUser.value.trim(),password=document.getElementById('login-password').value,remember=document.getElementById('remember-user').checked;loginError.textContent='Giriş yapılıyor…';try{const loginPayload=await apiFetch(endpoints.login,{method:'POST',body:JSON.stringify({email,password,remember})});accessToken=loginPayload.accessToken;if(remember&&email)localStorage.setItem('spaRememberedEmail',email);else localStorage.removeItem('spaRememberedEmail');await loadMembers();loginError.textContent='';loginScreen.classList.add('hidden');}catch(error){loginError.textContent=error.message;}});
