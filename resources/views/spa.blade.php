...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; background:var(--canvas); color:var(--ink); font:14px/1.4 "Segoe UI", system-ui, sans-serif; }
    ...
    .reservation-timeline { display:grid; grid-template-columns:62px 1fr; height:700px; overflow:hidden; background:#fffdf8; }
    .reservation-time-axis,.reservation-time-canvas { position:relative; height:100%; }
    .reservation-time-axis { color:#604510; border-right:1px solid #c9ad67; background:#f6ead0; font:9px Tahoma; }
    ...
    .reservation-time-canvas { min-width:560px; }
...
<body>
  ...
  <script>
    ...
    const reservationEventButton=item=>`<button type="button" class="reservation-event ${item.status}" data-reservation-id="${item.id}" title="${attr(item.service_name)} • ${attr(reservationEmployee(item))}">${attr(String(item.start_time).slice(0,5))} ${attr(reservationGuest(item))}</button>`;
    ...
    const reservationDayMarkup=()=>{const anchor=reservationParseDate(reservationData.anchorDate),date=localIso(anchor),items=reservationEventsFor(date).slice().sort((a,b)=>String(a.start_time).localeCompare(String(b.start_time))),opening=8*60,closing=22*60,total=closing-opening,toMinutes=value=>{const [hour,minute]=String(value).slice(0,5).split(':').map(Number);return hour*60+minute;},lanes=[],positioned=items.map(item=>{const start=toMinutes(item.start_time),end=toMinutes(item.end_time);let lane=lanes.findIndex(laneEnd=>laneEnd<=start);if(lane<0){lane=lanes.length;lanes.push(end);}else lanes[lane]=end;return{item,lane,start,end};}),laneCount=Math.max(lanes.length,1),labels=Array.from({length:15},(_,index)=>{const minute=opening+index*60,top=((minute-opening)/total)*100;return`<span class="reservation-time-label" style="top:${top}%">${String(Math.floor(minute/60)).padStart(2,'0')}:00</span>`;}).join(''),lines=Array.from({length:29},(_,index)=>`<span class="reservation-time-line ${index%2?'half':''}" style="top:${index/28*100}%"></span>`).join(''),events=positioned.filter(entry=>entry.end>opening&&entry.start<closing).map(({item,lane,start,end})=>{const top=(Math.max(start,opening)-opening)/total*100,height=(Math.min(end,closing)-Math.max(start,opening))/total*100,left=lane/laneCount*100,width=100/laneCount;return`<button type="button" class="reservation-day-event ${item.status}" data-reservation-id="${item.id}" style="top:${top}%;height:${height}%;left:calc(${left}% + 6px);width:calc(${width}% - 10px)"><strong>${attr(String(item.start_time).slice(0,5))}–${attr(String(item.end_time).slice(0,5))} • ${attr(reservationGuest(item))}</strong><span>${attr(item.service_name)} • ${attr(reservationEmployee(item))}</span><span>${attr(reservationStatusName(item.status))}</span></button>`;}).join('');return`${reservationPeriodHeader(reservationDateLabel(anchor,{weekday:'long',day:'2-digit',month:'long',year:'numeric'}))}<div class="reservation-day-view"><div class="reservation-day-title"><span>${items.length} rezervasyon • Boş saate sağ tıklayarak ekleyin</span><button type="button" data-reservation-add="${date}" title="Rezervasyon ekle">＋</button></div><div class="reservation-timeline"><div class="reservation-time-axis">${labels}</div><div class="reservation-time-canvas" data-reservation-date="${date}">${lines}${events||'<div class="reservation-empty">Bu gün için rezervasyon bulunmuyor.</div>'}</div></div></div>`;};
    ...
    const openAuxWindow = (id,title) => {
      ...
      if(id==='calendar'){reservationData.view='day';reservationData.anchorDate=localIso(new Date());setupReservationWindow(win);setupReservationContextMenu(win);}
