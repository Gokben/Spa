...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; background:var(--canvas); color:var(--ink); font:14px/1.4 "Segoe UI", system-ui, sans-serif; }
    ...
    .reservation-therapist-grid { --therapist-count:1; display:grid; grid-template-columns:62px repeat(var(--therapist-count),minmax(190px,1fr)); grid-template-rows:42px 700px; min-width:calc(62px + (var(--therapist-count) * 190px)); }
...
<body>
  ...
  <script>
    ...
    const setStartMenuOpen=open=>{startMenu.classList.toggle('open',open);startButton.setAttribute('aria-expanded',String(open));};
    ...
    const reservationDayMarkup=()=>{
      const anchor=reservationParseDate(reservationData.anchorDate),date=localIso(anchor),therapists=reservationTherapists(),therapistIds=new Set(therapists.map(item=>Number(item.id))),items=reservationEventsFor(date).filter(item=>therapistIds.has(Number(item.employee_id))).slice().sort((a,b)=>String(a.start_time).localeCompare(String(b.start_time))),opening=8*60,closing=22*60,total=closing-opening,toMinutes=value=>{const [hour,minute]=String(value).slice(0,5).split(':').map(Number);return hour*60+minute;},labels=Array.from({length:15},(_,index)=>{const minute=opening+index*60,top=((minute-opening)/total)*100;return`<span class="reservation-time-label" style="top:${top}%">${String(Math.floor(minute/60)).padStart(2,'0')}:00</span>`;}).join(''),lines=Array.from({length:29},(_,index)=>`<span class="reservation-time-line ${index%2?'half':''}" style="top:${index/28*100}%"></span>`).join(''),eventMarkup=item=>{const start=toMinutes(item.start_time),end=toMinutes(item.end_time),top=(Math.max(start,opening)-opening)/total*100,height=(Math.min(end,closing)-Math.max(start,opening))/total*100;return end>opening&&start<closing?`<button type="button" class="reservation-day-event ${item.status}" data-reservation-id="${item.id}" style="top:${top}%;height:${height}%;left:6px;width:calc(100% - 12px)"><strong>${attr(String(item.start_time).slice(0,5))}–${attr(String(item.end_time).slice(0,5))} • ${attr(reservationGuest(item))}</strong><span>${attr(item.service_name)}</span><span>${attr(reservationStatusName(item.status))}</span></button>`:'';};
      ...
      return`${reservationPeriodHeader(reservationDateLabel(anchor,{weekday:'long',day:'2-digit',month:'long',year:'numeric'}))}<div class="reservation-day-view"><div class="reservation-day-title"><span>${items.length} rezervasyon • ${therapists.length} terapist • Boş saate sağ tıklayarak ekleyin</span><button type="button" data-reservation-add="${date}" title="Rezervasyon ekle">＋</button></div><div class="reservation-timeline">${grid}</div></div>`;
    };
    ...
    const reservationMarkup=title=>`<div class="aux-titlebar"><span>${title}</span><div class="window-controls"><button type="button" data-act="min">_</button><button type="button" data-act="max">□</button><button type="button" class="window-close" data-act="close">×</button></div></div><div class="reservation-shell"><nav class="reservation-tabs" hidden><button type="button" class="active" data-reservation-tab="calendar"><span>▦</span>Takvim</button><button type="button" data-reservation-tab="form"><span>⊕</span>Yeni Rezervasyon</button><button type="button" data-reservation-tab="list"><span>▤</span>Rezervasyon Listesi</button></nav><div class="reservation-toolbar"><button type="button" data-reservation-command="new">⊕ Yeni</button><button type="button" data-reservation-command="save">💾 Kaydet</button><button type="button" data-reservation-command="refresh">🔄 Yenile</button><div class="reservation-view-switch" aria-label="Takvim görünümü"><button type="button" class="active" data-reservation-view="day">Günlük</button><button type="button" data-reservation-view="week">Haftalık</button><button type="button" data-reservation-view="month">Aylık</button></div><span class="reservation-message" role="status"></span></div><section class="reservation-panel" data-reservation-panel="calendar">Takvim yükleniyor…</section><section class="reservation-panel" data-reservation-panel="form" hidden>${reservationForm()}</section><section class="reservation-panel" data-reservation-panel="list" hidden>${reservationListMarkup()}</section></div>`;
    ...
    const openAuxWindow = (id,title) => {
      ...
      if(id==='calendar'){reservationData.view='day';reservationData.anchorDate=localIso(new Date());setupReservationWindow(win);setupReservationContextMenu(win);setupReservationDoubleClick(win);}
