@php($sofitelLogo = 'data:image/png;base64,'.base64_encode(file_get_contents(public_path('Sofitelspa-transparent.png'))))
<!doctype html>
<html lang="tr">
<head>
  <meta charset="utf-8">
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; background:var(--canvas); color:var(--ink); font:14px/1.4 "Segoe UI", system-ui, sans-serif; }
    ...
    .menu-brand:before { content:""; display:block; flex:0 0 40px; width:40px; height:32px; background:url('{{ $sofitelLogo }}') center/contain no-repeat; }
    ...
    .menu-logo { display:block; width:174px; height:112px; margin:13px auto 18px; background:url('{{ $sofitelLogo }}') center/contain no-repeat; }
    ...
    body:after { content:""; position:fixed; z-index:1; inset:0 0 31px 212px; pointer-events:none; background:url('{{ $sofitelLogo }}') center/min(58vw,760px) auto no-repeat; opacity:.12; }
...
<body>
  ...
  <script>
    ...
    const endpoints={login:@json(route('login')),logout:@json(url('/logout')),members:@json(url('/api/members')),workShifts:@json(url('/api/work-shifts')),employees:@json(url('/api/employees')),employeeSchedules:@json(url('/api/employee-schedules'))};
    ...
    const submenuDefinitions={'Personel':['Vardiya']};
    ...
    const openEmployeeCard=async employeeId=>{try{const employee=employeeId?(await apiFetch(`${endpoints.employees}/${employeeId}`)).data:{status:'aktif'},id=employeeId?`employee-${employeeId}`:'employee-new';employeeCardPayloads[id]=employee;openAuxWindow(id,employeeId?`Personel Kartı • ${employeeName(employee)}`:'Yeni Personel Kartı');}catch(error){alert(error.message);}};
    const setupEmployeeCardWindow=(win,id)=>{const form=win.querySelector('.employee-card-form'),status=win.querySelector('.employee-card-status'),photoInput=form.elements.photo_file,photoBox=win.querySelector('.employee-photo');win.querySelectorAll('[data-employee-tab]').forEach(button=>button.onclick=()=>{win.querySelectorAll('[data-employee-tab]').forEach(item=>item.classList.toggle('active',item===button));win.querySelectorAll('[data-employee-panel]').forEach(panel=>panel.hidden=panel.dataset.employeePanel!==button.dataset.employeeTab);});photoInput.onchange=()=>{const file=photoInput.files[0];if(!file)return;if(file.size>5*1024*1024){status.textContent='Fotoğraf en fazla 5 MB olabilir.';photoInput.value='';return;}const url=URL.createObjectURL(file);photoBox.innerHTML=`<img src="${url}" alt="Seçilen personel fotoğrafı">`;status.textContent='Fotoğraf kaydetmeye hazır.';};win.querySelector('[data-employee-card-action="close"]').onclick=()=>win.querySelector('[data-act="close"]').click();win.querySelector('[data-employee-card-action="terminate"]').onclick=()=>{form.elements.status.value='ayrıldı';form.elements.termination_date.value=form.elements.termination_date.value||new Date().toISOString().slice(0,10);};win.querySelector('[data-employee-card-action="ban"]').onclick=()=>form.elements.status.value='yasaklı';form.addEventListener('submit',async event=>{event.preventDefault();const employee=employeeCardPayloads[id],values=Object.fromEntries(new FormData(form));delete values.photo_file;status.textContent='Kaydediliyor…';try{const payload=await apiFetch(employee.id?`${endpoints.employees}/${employee.id}`:endpoints.employees,{method:employee.id?'PUT':'POST',body:JSON.stringify(values)});Object.assign(employee,payload.data);if(photoInput.files[0]){status.textContent='Fotoğraf yükleniyor…';const photoData=new FormData();photoData.append('photo',photoInput.files[0]);const photoPayload=await apiFetch(`${endpoints.employees}/${employee.id}/photo`,{method:'POST',body:photoData});Object.assign(employee,photoPayload.data);photoInput.value='';}status.textContent='Personel kaydı ve fotoğrafı kaydedildi.';const listWindow=document.querySelector('.aux-window[data-window-id="staff"]');if(listWindow)await loadEmployeeList(listWindow);}catch(error){status.textContent=error.message;}});};
    ...
    const setupMarkup=title=>`<div class="aux-titlebar"><span>${title}</span><div class="window-controls"><button type="button" data-act="min">_</button><button type="button" data-act="max">□</button><button type="button" class="window-close" data-act="close">×</button></div></div><div class="aux-content"><div class="setup-tabs"><button type="button" class="setup-tab active">Mesai Tanımları</button></div><section class="shift-panel"><div class="aux-toolbar"><button type="button" data-shift-action="new">⊕ Yeni Mesai</button><button type="button" data-shift-action="refresh">🔄 Yenile</button><span class="shift-message" role="status"></span></div><div class="shift-grid-wrap"><table class="aux-grid shift-table"><thead><tr><th>Kod</th><th>Mesai Saatleri</th><th>İşlemler</th></tr></thead><tbody><tr><td colspan="3">Mesailer yükleniyor…</td></tr></tbody></table></div></section></div>`;
    ...
    const scheduleMarkup=title=>`<div class="aux-titlebar"><span>${title}</span><div class="window-controls"><button type="button" data-act="min">_</button><button type="button" data-act="max">□</button><button type="button" class="window-close" data-act="close">×</button></div></div><div class="schedule-shell"><div class="schedule-toolbar"><button type="button" data-schedule-action="previous">◀ Önceki Hafta</button><button type="button" data-schedule-action="today">Bu Hafta</button><strong class="schedule-week-label">Hafta yükleniyor…</strong><button type="button" data-schedule-action="next">Sonraki Hafta ▶</button><button type="button" data-schedule-action="save">💾 Haftayı Kaydet</button><span class="shift-message" role="status"></span></div><div class="schedule-grid-wrap"><table class="schedule-table"><tbody><tr><td class="schedule-empty">Personel ve mesailer yükleniyor…</td></tr></tbody></table></div></div>`;
    const scheduleChoice=(assignment,shifts)=>{const value=assignment?.work_shift_id?`shift:${assignment.work_shift_id}`:(assignment?.status||'');return `<option value="">— Seçiniz —</option>${shifts.map(shift=>`<option value="shift:${shift.id}" ${value===`shift:${shift.id}`?'selected':''}>${attr(shortTime(shift.start_time))}-${attr(shortTime(shift.end_time))}</option>`).join('')}<option value="off" ${value==='off'?'selected':''}>OFF</option><option value="izin" ${value==='izin'?'selected':''}>İZİN</option><option value="raporlu" ${value==='raporlu'?'selected':''}>RAPORLU</option>`;};
    const renderSchedule=(win,data)=>{scheduleWeekStart=data.week_start;const start=new Date(`${data.week_start}T12:00:00`),days=Array.from({length:7},(_,index)=>{const day=new Date(start);day.setDate(start.getDate()+index);return day;}),dayNames=['Pazartesi','Salı','Çarşamba','Perşembe','Cuma','Cumartesi','Pazar'],assignmentMap=new Map(data.assignments.map(item=>[`${item.employee_id}:${String(item.work_date).slice(0,10)}`,item]));win.querySelector('.schedule-week-label').textContent=`${displayDate(localIso(days[0]))} – ${displayDate(localIso(days[6]))}`;const head=`<thead><tr><th>PERSONEL</th>${days.map((day,index)=>`<th><span class="schedule-day-name">${dayNames[index]}</span><span class="schedule-day-date">${displayDate(localIso(day))}</span></th>`).join('')}</tr></thead>`;const body=data.employees.length?data.employees.map(employee=>`<tr><th>${attr(employeeName(employee))}</th>${days.map(day=>{const date=localIso(day),assignment=assignmentMap.get(`${employee.id}:${date}`),state=assignment?.status?` state-${assignment.status}`:'';return `<td class="schedule-cell${state}"><select data-employee-id="${employee.id}" data-work-date="${date}" aria-label="${attr(employeeName(employee))} ${displayDate(date)}">${scheduleChoice(assignment,data.work_shifts)}</select></td>`;}).join('')}</tr>`).join(''):`<tr><td colspan="8" class="schedule-empty">Aktif personel kaydı bulunamadı. Önce Personel ekranından aktif personel ekleyin.</td></tr>`;win.querySelector('.schedule-table').innerHTML=head+`<tbody>${body}</tbody>`;};
    const setupScheduleWindow=win=>{const message=win.querySelector('.shift-message');const load=async week=>{message.textContent='Plan yükleniyor…';try{const payload=await apiFetch(`${endpoints.employeeSchedules}?week_start=${week}`);renderSchedule(win,payload.data);message.textContent=`${payload.data.employees.length} personel • ${payload.data.work_shifts.length} mesai`;}catch(error){message.textContent=error.message;}};const move=days=>{const date=new Date(`${scheduleWeekStart}T12:00:00`);date.setDate(date.getDate()+days);load(localIso(date));};win.querySelector('[data-schedule-action="previous"]').onclick=()=>move(-7);win.querySelector('[data-schedule-action="next"]').onclick=()=>move(7);win.querySelector('[data-schedule-action="today"]').onclick=()=>load(localIso(mondayOf(new Date())));win.querySelector('[data-schedule-action="save"]').onclick=async()=>{const assignments=[...win.querySelectorAll('.schedule-cell select')].map(select=>{const value=select.value;return {employee_id:Number(select.dataset.employeeId),work_date:select.dataset.workDate,work_shift_id:value.startsWith('shift:')?Number(value.slice(6)):null,status:value&&!value.startsWith('shift:')?value:null};});message.textContent='Hafta kaydediliyor…';try{const payload=await apiFetch(endpoints.employeeSchedules,{method:'PUT',body:JSON.stringify({week_start:scheduleWeekStart,assignments})});renderSchedule(win,payload.data);message.textContent='Haftalık çalışma planı kaydedildi.';}catch(error){message.textContent=error.message;}};win.querySelector('.schedule-grid-wrap').addEventListener('change',event=>{const select=event.target.closest('.schedule-cell select');if(!select)return;select.parentElement.className=`schedule-cell${['off','izin','raporlu'].includes(select.value)?` state-${select.value}`:''}`;});load(localIso(mondayOf(new Date())));};
    ...
    const openAuxWindow = (id,title) => {
      ...
      if(id==='setup')setupWorkShiftWindow(win);
