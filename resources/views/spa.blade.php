...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; background:var(--canvas); color:var(--ink); font:14px/1.4 "Segoe UI", system-ui, sans-serif; }
    ...
    .desktop-status img { display:block; width:100%; height:auto; }
    ...
    .menu-logo { display:block; width:136px; height:72px; margin:10px auto 12px; background:url('{{ asset('Sofitelspa-transparent.png') }}') center/contain no-repeat; }
    ...
    body:after { content:""; position:fixed; z-index:1; inset:0 0 31px; pointer-events:none; background:url('{{ asset('Sofitelspa-transparent.png') }}') center/min(54vw,620px) auto no-repeat; opacity:.16; }
    ...
    @media (max-width:900px) { .desktop-icons,.desktop-status { display:none; } .app { top:8px; left:8px; right:8px; width:auto; height:calc(100vh - 47px); } .field-grid { grid-template-columns:repeat(2,minmax(140px,1fr)); } }
...
<body>
  <div class="login-screen" id="login-screen"><form class="login-panel" id="login-form"><img class="login-logo" src="{{ asset('Sofitelspa-transparent.png') }}" alt="Sofitel Spa"><h1 class="login-title">Kullanıcı Girişi</h1><p class="login-subtitle">Şimdilik bilgileri doldurmadan devam edebilirsiniz.</p><div class="login-field"><label for="login-user">E-posta <small>(opsiyonel)</small></label><input id="login-user" name="email" type="email" autocomplete="username" autofocus></div><div class="login-field"><label for="login-password">Şifre <small>(opsiyonel)</small></label><input id="login-password" name="password" type="password" autocomplete="current-password"></div><div class="login-options"><label><input type="checkbox" id="remember-user">E-postayı hatırla</label><span>Geçici erişim</span></div><button class="login-submit" type="submit">Giriş Yap</button><p class="login-error" id="login-error" role="alert"></p></form></div>
  ...
  <script>
    ...
    const endpoints={login:@json(route('login')),logout:@json(url('/logout')),members:@json(url('/api/members'))};
    const apiFetch=async(url,options={})=>{const response=await fetch(url,{credentials:'same-origin',headers:{'Accept':'application/json','Content-Type':'application/json','X-CSRF-TOKEN':csrfToken,...(options.headers||{})},...options});const payload=await response.json().catch(()=>({message:'Sunucu yanıtı okunamadı.'}));if(!response.ok)throw new Error(payload.message||Object.values(payload.errors||{}).flat()[0]||'İşlem tamamlanamadı.');return payload;};
    let memberRecords=[];
    ...
    loginForm.addEventListener('submit',async event=>{event.preventDefault();const email=loginUser.value.trim(),password=document.getElementById('login-password').value,remember=document.getElementById('remember-user').checked;loginError.textContent='Giriş yapılıyor…';try{await apiFetch(endpoints.login,{method:'POST',body:JSON.stringify({email,password,remember})});if(remember&&email)localStorage.setItem('spaRememberedEmail',email);else localStorage.removeItem('spaRememberedEmail');await loadMembers();loginError.textContent='';loginScreen.classList.add('hidden');}catch(error){loginError.textContent=error.message;}});
    ...
    startMenu.insertAdjacentHTML('afterbegin','<span class="menu-logo" aria-label="VOX İşitme Cihazları"></span>');
    ...
    Object.entries(submenuDefinitions).forEach(([parentLabel,children])=>{
      ...
      parent.addEventListener('click',event=>{event.preventDefault();event.stopImmediatePropagation();const open=!submenu.classList.contains('open');submenu.classList.toggle('open',open);parent.classList.toggle('expanded',open);});
    });
    ...
    document.getElementById('logout-button').addEventListener('click',async event=>{event.preventDefault();try{const payload=await apiFetch(endpoints.logout,{method:'POST',body:'{}'});csrfToken=payload.csrfToken;document.querySelector('meta[name="csrf-token"]').content=csrfToken;}finally{memberRecords=[];document.getElementById('start-menu').classList.remove('open');document.getElementById('login-password').value='';loginScreen.classList.remove('hidden');setTimeout(()=>loginUser.focus(),330);}});
    ...
    const openAuxWindow = (id,title) => {
      ...
      win=document.createElement('section'); win.className='aux-window'; win.dataset.windowId=id; win.innerHTML=auxMarkup(id,title); document.body.appendChild(win);if(id==='listeler'){win.style.width='880px';win.style.height='480px';}if(id.startsWith('member-')){win.style.width='900px';win.style.height='530px';}
      ...
      const closeWindow=()=>{task.remove();win.remove();if(id.startsWith('member-'))delete memberCardPayloads[id];};
