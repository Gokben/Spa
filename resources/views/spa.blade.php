...
<body>
  ...
  <script>
    ...
    const endpoints={login:@json(route('login')),logout:@json(url('/logout')),members:@json(url('/api/members')),workShifts:@json(url('/api/work-shifts')),businessHours:@json(url('/api/business-hours')),employees:@json(url('/api/employees')),employeeSchedules:@json(url('/api/employee-schedules'))};
    ...
    const setupSetupWindow=win=>{
      ...
      win.querySelectorAll('[data-setup-tab]').forEach(tab=>tab.onclick=()=>{
        ...
        if(tab.dataset.setupTab==='business-hours'&&!businessHoursLoaded){businessHoursLoaded=true;setupBusinessHoursWindow(win);}
      });
    };
